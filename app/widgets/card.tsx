...
const Card: FC<CardProps> = ({ title, subtitle, description, image }) => {
  return (
    <div className="relative w-[455px] bg-[#13171d99] group rounded-md cursor-pointer">
      <div className="rounded-full bg-white text-black text-[12px] absolute top-0 px-2 transform -translate-y-1/2 right-[5%]">
        <span className="uppercase">coming soon</span>
      </div>
      <h2 className={`${styles.subtitle} p-5`}>{title}</h2>
      <div className={`${styles.comment} p-5`}>
        <span>{subtitle}</span>
      </div>
        ...
        <div className="w-full overflow-hidden rounded-r-lg relative my-5">
          ...
          <div className="w-[95%] overflow-hidden rounded-r-lg">
            <Image
              src={image}
              alt={title}
              className="w-full object-cover overflow-hidden transition-transform duration-[1s] ease-in-out transform group-hover:scale-[1.2]"
            />
          </div>
        </div>
      </div>
      <p className={`${styles.paragraph} mt-[300px] p-5`}>
        {description}
      </p>
    </div>
  );
